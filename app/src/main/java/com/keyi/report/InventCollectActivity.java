...
import com.keyi.report.adpater.InventCollectAdapter;
 ...
 */
public class InventCollectActivity extends Activity implements MsgInterface {
    private XRecyclerView tableListView;
    List<InventCollectMsg.DataBean> dataBeans;
    private int page=0;
    InventCollectAdapter adapter;
    @Override
    protected void onCreate(Bundle savedInstanceState) {
        super.onCreate(savedInstanceState);
        setContentView(R.layout.activity_inventcollect);
    ...
    private void initData() {
        ...
        final String url = DatasUtils.inventCollectUrlSting(InventCollectActivity.this, page);
    ...
    public void isTrue(String istrue, int queneFlag) {
        page++;
        Gson gson=new Gson();
        InventCollectMsg inventCollectMsg=gson.fromJson(istrue,InventCollectMsg.class);
        if (inventCollectMsg.isIsOK()){
            if (queneFlag==0){
                for (int i=0;i<inventCollectMsg.getData().size();i++){
                    dataBeans.add(inventCollectMsg.getData().get(i));
                }
                adapter = new InventCollectAdapter(dataBeans);
            ...
            Toast.makeText(InventCollectActivity.this, "已全部加载完", Toast.LENGTH_SHORT).show();
            tableListView.loadMoreComplete();
        }
    }
    ...
    public void isError(String isError, int queneFlag) {
        Toast.makeText(InventCollectActivity.this, "网络或服务器异常！", Toast.LENGTH_SHORT).show();
    }
}
